...
<Callout type="info" title="Note">
    Each Faction Title is configurable in the language configuration files.\
...
<Callout type="info" title="Note">
    Although permissions are hardcoded for each rank / title and cannot change, **the names of each Title can be modified in the language files.**
</Callout>